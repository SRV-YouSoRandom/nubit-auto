...
# Step 2: Update the system and install curl, wget, and git if needed
...
git clone -b nubit-alphatestnet-1 https://github.com/RiemaLabs/nubit-node.git /home/nubit/nubit-node
if [[ $? -ne 0 ]]; then
...
sudo rm -rf $NUBIT_DATA_HOME/blocks $NUBIT_DATA_HOME/data $NUBIT_DATA_HOME/index $NUBIT_DATA_HOME/inverted_index $NUBIT_DATA_HOME/transients
...
sudo tar -zxf lightnode_data.tgz -C $NUBIT_DATA_HOME
if [[ $? -ne 0 ]]; then
    echo 'Error: Failed to extract the snapshot!'
    exit 1
fi
sudo rm -f lightnode_data.tgz
